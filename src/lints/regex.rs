...
regex_line_lint!(TrailingSpace, "LXX-002", "No trailing space allowed", r"\s$");
...
regex_global_lint!(HyphenTwoLines, "NX05-001-1", "Hyphen should be used only when two speakers", r"^-.*\n[^-]|^[^-].*\n-");
regex_line_lint!(HyphenWithoutSpace, "NX05-001-2", "Use an English hyphen without a space", r"^-(\s|$)");
regex_line_lint!(NoFullWidthNumber, "NX11-002", "No full-width number allowed", r"[\u{ff10}-\u{ff19}]");
regex_line_lint!(NoCommaOrPeriod, "NX12-001", "No comma or period allowed", r"[.,]");
regex_line_lint!(NoU22EFEllipsis, "NX12-008", "No U+22EF ellipsis allowed", r"\u{22EF}");
regex_line_lint!(NoDuplicatedPunctuation, "NX12-009", "No duplicated punctuation allowed", r"[.,?!？…！]{2,}");
